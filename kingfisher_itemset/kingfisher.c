...
int prunecandpF(Node* t,Node *p,bitvector** amatr,int l,int* fr,int *kandi, int *order, int *oldaddr, int* row_lengths);
...
// ADDED
/* used in frset to check if a target attribute is present in the row */
...
// ADDED
/* frequency counting given the transaction matrix */
int frset(int *set, int len, bitvector **amatr, int *row_lengths) {
  int i, fr = 0;
...
int visit(Node* p, Node* t,int len, int l, int* next, int** kandi,int* order,bitvector **amatr,int *fr,int *oldaddr, int* row_lengths){
  int i,j;
  Node *child;
  int *par2set;
  ...
  if (len==l-2){ /* path length l-2, i.e. we are in a leaf (do nothing) or */
	...
	if (p->children[i]!=NULL){
	  for (j=i+1; j<(p->chsize); j++)
		...
		if (checkpar(*kandi,l,t,child,order,fr)==0){
		  ...
		  if (prunecand(t,child,amatr,l,fr,*kandi,order,oldaddr, row_lengths)==0){
    ...
    for (i=0; i<p->chsize; i++){ /* polku kesken, jos lapsia. */
      if (p->children[i]!=NULL){
	(*kandi)[len]=p->children[i]->label;
	if ((visit(p->children[i],t,len+1,l,next,kandi,order,amatr,fr,oldaddr, row_lengths)==0)&&(len>0))
	  delChild(p,p->children[i]->label,order);
      }
    } /* for */
...
int intervisit(Node* p, Node* t,int len, int l, int* next, int** kandi,int* order,bitvector **amatr,int *fr,int *oldaddr, int iinpath, int* row_lengths){
  int i,j;
  Node *child;
  int *par2set;
  int oldiinpath;
  ...
  if (len==l-2){ /* path length l-2, i.e. we are in a leaf (do nothing) or */
	...
	if (interattr[p->children[i]->label]) iinpath=1;
	(*kandi)[len]=p->children[i]->label;
	if ((intervisit(p->children[i],t,len+1,l,next,kandi,order,amatr,fr,oldaddr,iinpath, row_lengths)==0)&&(len>0))
	  delChild(p,p->children[i]->label,order);
	iinpath=oldiinpath;
      }
    } /* for */
...
int generatecandidates(Node* t,int l,int* order,bitvector **amatr,int *fr,int *oldaddr, int* row_lengths){
  int i,iinpath=0;
  int* kandi;
  int next=0;
  float prevvalM=valM;
  ...
  if (l==2){ /* level 2 */
    if (intergiven==0)
      visit(t,t,0,l,&next,&kandi,order,amatr,fr,oldaddr, row_lengths);
    else
      intervisit(t,t,0,l,&next,&kandi,order,amatr,fr,oldaddr,iinpath, row_lengths);
	...
	if (intergiven==0)
	  visit(t->children[i],t,1,l,&next,&kandi,order,amatr,fr,oldaddr, row_lengths);
	else {
	  if (interattr[t->children[i]->label])
	    intervisit(t->children[i],t,1,l,&next,&kandi,order,amatr,fr,oldaddr,1, row_lengths);
	  else intervisit(t->children[i],t,1,l,&next,&kandi,order,amatr,fr,oldaddr,0, row_lengths);
	}
	if (better(valM,prevvalM)){
	  updatevalM(prevvalM,valM,fr,oldaddr);
	  prevvalM=valM;
...
int prunecandpF(Node* t,Node *p,bitvector** amatr,int l,int* fr,int *kandi, int *order, int *oldaddr, int* row_lengths){
  ...
  /* count frequency */
  p->data->fr=frset(kandi,l,amatr, row_lengths);
...
int prunecandM(Node* t,Node *p,bitvector** amatr,int l,int* fr,int *kandi, int *order, int *oldaddr, int* row_lengths){
  int i,fra,fry;
  int cf1found=0; /* was any cf==1? */
  int oind,oind2;
  int indinkandi,indinkandi2;
...
int main(int argc,char**argv){
  ...
  initattrmatr(f, &amatr, &row_lengths, n,k);
  if(fclose(f)==EOF)
    error(4);
  /*  printdata(matr,n,k);*/
  ...
  while ((numl>=l)&&(l<=MAXTASO)){
    ...
    tim1=clock();
    numl=generatecandidates(t,l,order,amatr,fr,oldaddr, row_lengths);
    tim2=clock();
    tottim+=(double)(tim2-tim1);
  ...
  time_end = clock();
  total_main_time = time_end - time_start;
  total_main_time/=CLOCKS_PER_SEC;
  ...
  //printf("Total execution time %.0f s=%.0f min (%.6f s)\n",total_main_time,total_main_time/60,total_main_time);
